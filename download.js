...
function downloadSession(format, output) {
  let config = require(program.config),
    api = new ConfTool.Client({
      apiKey: config.api_key,
      baseUrl: config.api_base_url
  ...
  log('Using format: ' + format);
  log('Trying to download session data from ' + config.api_base_url);
  api.downloadSessions({
    form_export_format: format,
    extended: ['presentations', 'presentations_abstracts', 'presentations_downloads', 'presentations_authors_extended', 'newlines']
  }).then(function(result) {
    log('Finished downloading. Saving output to ' + output);
    writeToFile(result, output);
  });
...
(function run() {
  switch (program.target) {
    case 'paper':
      ...
      break;
    case 'sessions':
      downloadSession(program.format, program.output);
      break;
    default:
